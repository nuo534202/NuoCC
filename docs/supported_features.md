...
1. `char`, `int` and `long` variable Declaration and Assignment. A `char`
   holds 0 to 255, an `int` four bytes and a `long` eight. A narrower value
   is widened where the types are mixed; a value which would have to be
